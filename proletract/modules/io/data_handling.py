...
class VCFHandler:
    ...
    def parse_vcf_Vamos(self, vcf_file):
        vcf = pysam.VariantFile(vcf_file)
        records_map = {}
        records_ids = {}
        idx = 0
        for rec in vcf.fetch():
            records_ids[f"{rec.chrom}:{rec.pos}-{rec.stop}"] = f"{rec.chrom}:{rec.pos}-{rec.stop}"
            records_map[idx] = f"{rec.chrom}:{rec.pos}-{rec.stop}"
            idx += 1
        return records_ids, records_map
    ...
    def handle_comparison_samples(self):
        ...
        if 'vcf_file_vamos_input' not in st.session_state:
            st.session_state.vcf_file_vamos_input = None
        ...
        default_vamos = "/confidential/Structural_Variation//Lion/TandemTwist/MC/HG002_vamos_CCS.vcf.gz"
        ...
        if 'vcf_file_vamos_input' not in st.session_state:
            st.session_state.vcf_file_vamos_input = default_vamos
        ...
        # To ensure the default value appears, explicitly fetch from session_state at render time
        vcf_file_vamos = st.sidebar.text_input(
            "Enter the path of your Vamos VCF file",
            key="vcf_file_vamos_input",
            help="Enter the path of your Vamos VCF file, the file should be zipped and indexed with tabix",
            value=st.session_state.get("vcf_file_vamos_input", default_vamos)
        )
        ...
        if middle.button("Load Comparison Samples"):
            if vcf_file_trgt and vcf_file_tandemtwister and assembly_vcf_h1 and assembly_vcf_h2:
                ...
                st.session_state.vcf_file_vamos = vcf_file_vamos
                ...
                st.session_state.pop('vcf_file_vamos_records', None)
                st.session_state.pop('vcf_file_vamos_records_map', None)
            else:  
            ...
            if 'vcf_file_trgt_records' not in st.session_state:
                try:
                    ...
                    st.info("TRGT VCF file parsed successfully")
                except:
            ...
            if 'vcf_file_tandemtwister_records' not in st.session_state:
                ...
                try:
                    ...
                    st.info("TandemTwister VCF file parsed successfully")
                except:
                    info_container.warning("Failed to parse the TandemTwister VCF file")
            if 'vcf_file_vamos_records' not in st.session_state:
                info_container = st.container()
                try:
                    st.session_state.vcf_file_vamos_records, st.session_state.vcf_file_vamos_records_map = self.parse_vcf_Vamos(st.session_state.vcf_file_vamos)
                    st.info("Vamos VCF file parsed successfully")
                except:
                    info_container.warning("Failed to parse the Vamos VCF file")    
            if 'assembly_vcf_records_h1' not in st.session_state:
                ...
                try:
                    ...
                    st.info("Assembly VCF file for haplotype 1 parsed successfully")
                except:
            ...
            if 'assembly_vcf_records_h2' not in st.session_state:
                ...
                try:
                    ...
                    st.info("Assembly VCF file for haplotype 2 parsed successfully")
                except:
